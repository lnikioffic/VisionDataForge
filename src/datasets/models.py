...
class Dataset(Base):
    ...
    first_frame: Mapped[str] = mapped_column(String(200))
    second_frame: Mapped[str] = mapped_column(String(200))
    size: Mapped[str]
    ...
    type_dataset_id: Mapped[int] = mapped_column(
        ForeignKey('type_dataset.id', onupdate='CASCADE', ondelete='RESTRICT')
    )
...
class TypeDataset(Base):
    ...
    datasets_for_type: Mapped[list['Dataset']] = relationship(
        back_populates='type_dataset'
    )
